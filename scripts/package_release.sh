#!/usr/bin/env bash
# Package a Final Fantasy V release: a SETUP PACK.
#
...
#
# Writes: dist/ffvjp-<VERSION>-<platform-tag>.zip
set -euo pipefail
...
# mods/ is the catalog the build stages beside the executable, and the package
# a netplay host transfers to a guest. Add --runtime-dir <name> for any other
# directory the game loads exe-relative (translations, ...).
exec bash "$IMPL" \
  --runtime-dir mods \
  --build-dir "$BUILD_DIR" \
  --artifact "$PLATFORM" \
  --exe-name FinalFantasyVSNESRecomp \
  --zip-prefix ffvjp \
  --display-name "Final Fantasy V" \
  --rom-hint "$ROM_FILE" \
